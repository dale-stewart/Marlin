...
| 26 | A peripheral registered with `Gpio::attachPeripheral()` outlives the object it points at. AddressSanitizer reports a **stack-use-after-return** on the first probing move: `Stepper::pulse_phase_isr()` writes a step pin, `Gpio::set()` dispatches to the registered peripheral, and the `SimulatedHeater` it reaches was a stack local of an earlier test whose memory is now reused (ASan names the reusing frame as `Endstops::update()`). The heater's own destructor *does* detach, so the stale registration is on some other path — not yet located. Without ASan the symptom is a stack smash inside whichever unrelated test is running at the time, with a corrupted return address; that is what it did to the first `probe_at_point()` test, which looked like a probing fault and was not. | `tests/support/simulated_heaters.h`, `HAL/TEST/hardware/Gpio.h`, some test that constructs a heater | Blocks probing tests, and silently endangers every test that runs after a heater test. `simulated_endstops.h` already carries a note about exactly this hazard in `HAL/TEST/hardware/LinearAxis`, so the class of fault was known; this is a second instance in the fixtures themselves. Reproduce with `make unit-test-asan UNIT_TEST_CONFIG=bed_leveling`. | open, harness |

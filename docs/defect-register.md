...
| # | Behaviour | Where |
|---|---|---|
| 8 | A value too wide for its field is truncated to the low digits with no marker: `ui16tostr3rj(1000)` is `"000"`, `ui8tostr2(100)` is `"00"`, `ftostr72rj(123456.7)` is `"23456.70"`. | `libs/numtostr.cpp` |
| 9 | The `ns` (no sign) conversions drop the sign, so `-1.5` renders identically to `1.5`. Likewise `ftostr5rj`. | `libs/numtostr.cpp` |
| 10 | `pcttostrpctrj()` does not clamp, so a value above 100 prints as-is (`123%`). | `libs/numtostr.cpp:73` |
| 11 | `ftostr52sprj()` and `utostr3()` *clamp* at their maximum while everything else truncates — an inconsistency within the library, not a fault in either. | `libs/numtostr.cpp` |
...
|---|---|---|
| 12 | `GCodeParser` is static-only with global mutable state (`codenum`, `codebits`, `string_arg`), read directly across the codebase. | its consumers are rescued — Phase 2 |
| 13 | Every `numtostr` conversion writes into one shared `char conv[9]` and returns a pointer into it, so two calls in one expression clobber each other. Fan-in of 23 files. | its consumers are rescued |

...
| 31 | **`M92 E<low>` compensates the acceleration limit and the compensation is immediately discarded.** A suspiciously low E resolution (below 20 steps/mm) is treated as a change of units rather than of speed, and `M92.cpp:66-67` scales both `settings.max_feedrate_mm_s[E]` and `planner.max_acceleration_steps_per_s2[E]` by the ratio. Two lines later `planner.refresh_positioning()` calls `refresh_acceleration_rates()`, which recomputes that array from `max_acceleration_mm_per_s2 * axis_steps_per_mm` for every axis unconditionally. The write is overwritten before anything can read it — deleting line 67 entirely leaves the whole suite passing. | `gcode/config/M92.cpp:67` | The feedrate half works and is what the compensation is for: it holds the extruder speed limit constant in steps/s. The acceleration half is dead, and what it intends is unclear — the factor it applies preserves neither the mm/s^2 figure nor the steps/s^2 one. The current outcome (mm/s^2 untouched, steps/s^2 following the new resolution) is the defensible one, so this is recorded rather than changed. Pinned by `the_low_M92_E_acceleration_compensation_is_discarded`. | open, dead code |

...
| 26 | A peripheral registered with `Gpio::attachPeripheral()` is called after the object it points at is gone. Under AddressSanitizer, the first probing move gives a **stack-use-after-return**: `Stepper::pulse_phase_isr()` writes a pin, `Gpio::set()` dispatches to the registered peripheral, and it reaches a `SimulatedHeater` whose storage is a returned stack frame. Reproduced deterministically; full trace in the commit that recorded this. **Narrowed since:** the pin table is now checked against its starting state after every test (`harness___no_test_left_the_pin_table_disturbed`) and is clean at every boundary in every configuration — so the stale pointer is not left behind by an earlier test, it appears *during* the probing test, which then dies before the boundary check can run. `Gpio::set()` does validate its pin, so it is not an out-of-range index. What plants the pointer is still unknown. Without a sanitizer the symptom is a stack smash inside whichever test is running, with a corrupted return address, which is what made the first `probe_at_point()` test look like a probing fault when it was not. | `tests/support/simulated_heaters.h`, `HAL/TEST/hardware/Gpio.h`, the probing path | Blocks the probing tests, and `probe.cpp` and `G29` therefore stay at 0%. The fixture they need (`tests/support/simulated_bed.h`) is written and tested. Reproduce with `make unit-test-asan UNIT_TEST_CONFIG=bed_leveling` after restoring the `probe_at_point` test. `SimulatedHeater::interrupt()` also ignores which pin fired, unlike `SimulatedAxisWithLimit::interrupt()` which checks — not the cause, but worth correcting when this is. | open, harness |

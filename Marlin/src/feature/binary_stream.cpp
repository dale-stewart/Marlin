...
static bool bs_serial_data_available(const serial_index_t index) {
  return SERIAL_IMPL.available(index);
}

static int bs_read_serial(const serial_index_t index) {
  return SERIAL_IMPL.read(index);
}

/**
 * The file half of the protocol: what a payload packet means once its frame has been verified.
 *
 * Referenced only by BinaryStream::dispatch(), so it lives here rather than in the header.
 */
...
uint8_t BinaryStream::Packet::Header::protocol() { return (meta >> 4) & 0xF; }
uint8_t BinaryStream::Packet::Header::type() { return meta & 0xF; }
void BinaryStream::Packet::Header::reset() { token = 0; sync = 0; meta = 0; size = 0; checksum = 0; }

void BinaryStream::Packet::Footer::reset() { checksum = 0; }
...
uint32_t BinaryStream::checksum(uint32_t cs, uint8_t value) {
...
void BinaryStream::receive(char * const buffer, const size_t buffer_size) {
  uint8_t data = 0;
          ...
          if (packet.header.checksum == packet.header_checksum) {
            ...
            if (packet.header.sync == sync) {
              ...
              if (packet.header.size) {
                ...
                packet.buffer = buffer; // multipacket buffering not implemented, always allocate whole buffer to packet
              }
              else

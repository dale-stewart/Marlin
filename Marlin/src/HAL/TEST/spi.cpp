 ...
 * card on it. `DiskIODriver` is a pure virtual interface and `CardReader::changeMedia()`
 * is public, so a fake block device is injected at the level the firmware already
 * abstracts, and none of the code below is on that path. Simulating a card at the wire
 * level would mean implementing SD's command protocol to test code that sits above it.
 */

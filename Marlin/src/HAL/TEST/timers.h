...
// Advance simulated time, firing any interrupts that fall inside it. Time passes only
// when a test asks it to; see hardware/Clock.h.
void HAL_test_advance_nanos(const uint64_t ns);
void HAL_test_advance_micros(const uint32_t us);
void HAL_test_advance_millis(const uint32_t ms);

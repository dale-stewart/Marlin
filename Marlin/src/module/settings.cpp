  ...
  EEPROM_Error MarlinSettings::_load() {
    ...
    do { // A block to break out of on error
      ...
      //
      {
        ...
        if (!validating) {
          uint32_t accel[DISTINCT_AXES];
          feedRate_t feedrate[DISTINCT_AXES];
          #if ENABLED(EDITABLE_STEPS_PER_UNIT)
            float steps_per_mm[DISTINCT_AXES];
          #endif
          LOOP_DISTINCT_AXES(i) {
            const bool in = (i < e_factors + NUM_AXES);
            accel[i]        = in ? tmp1[i] : pgm_read_dword(&_DMA[ALIM(i, _DMA)]);
            TERN_(EDITABLE_STEPS_PER_UNIT, steps_per_mm[i] = in ? tmp2[i] : pgm_read_float(&_DASU[ALIM(i, _DASU)]));
            feedrate[i]     = in ? tmp3[i] : pgm_read_float(&_DMF[ALIM(i, _DMF)]);
          }
...
void MarlinSettings::reset() {
  ...
  #endif
  LOOP_DISTINCT_AXES(i) {
    accel[i]    = pgm_read_dword(&_DMA[ALIM(i, _DMA)]);
